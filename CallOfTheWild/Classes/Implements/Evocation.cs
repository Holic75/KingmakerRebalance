...
namespace CallOfTheWild
{
    public partial class ImplementsEngine
    {
        ...
        public BlueprintFeature createEnergyBlast()
        {
            ...
            int dmg_bonus = Main.settings.balance_fixes ? 3 : 2;
            var custom_progression = new (int, int)[0];
            for (int i = 2;  i <= 30; i+= 2)
            {
                custom_progression = custom_progression.AddToArray((dmg_bonus * (i / 2), i));
            }
            for (int i = 0; i < abilities.Length; i++)
            {
                ...
                var ability = Helpers.CreateAbility(prefix + names[i] + "EnergyBlastAbility",
                                                    ...
                                                    $"As a standard action that provokes attacks of opportunity, you can expend 2 points of mental focus to unleash a blast of energy. This blast has a range of 60 feet, and deals 1d{BalanceFixes.getDamageDieString(DiceType.D6)}+{dmg_bonus} points of energy damage, plus an additional 1d{BalanceFixes.getDamageDieString(DiceType.D6)}+{dmg_bonus} points for every 2 occultist levels you possess beyond 1st. The blast deals damage to each creature in a 20-foot-radius burst, but each affected creature can attempt a Reflex save to halve the damage. When you unleash an energy blast, you must decide what type of damage it deals (acid, cold, electricity, or fire). You must be at least 5th level to select this focus power.",
                                                    "",
                                                    icons[i],
                                                    AbilityType.SpellLike,
                                                    CommandType.Standard,
                                                    ...
                                                    createClassScalingConfig(ContextRankProgression.Custom, type: AbilityRankType.DamageBonus, 
                                                                             customProgression: custom_progression),
